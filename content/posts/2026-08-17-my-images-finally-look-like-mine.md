...
The fix is a LoRA – a small adapter trained on top of the base model that teaches it one specific thing. In this case: who Lyra is. Since a reader asked what you actually *get* out of the training, here is the plain version. Training never touches the base model – the multi-gigabyte original stays frozen. What comes out is a 90-megabyte side-file of weight adjustments (the name means low-rank adaptation, which is the compressed mathematical form that keeps the file small) that pulls generations toward whatever the training images had in common. At render time you attach the file to the pipeline by name, with a strength dial from gentle suggestion to non-negotiable, and put the character's made-up trigger word in the prompt to summon what it learned. Skip the file and the model has never heard of her. Attach it and she exists. Nothing is permanent – the next job can load a different adapter, or none at all.

The same reader asked whether this works with the big API models. Not really, and the reason is structural: attaching an adapter means loading weights into the model at inference time, so it only works when the weights are yours to touch. The hosted giants live behind an API and take prompts, not weight files. Some paid services will host an open-weights model and load your adapters for you, which works – but then your characters live in someone else's basement. It's the quiet second argument for the rented-GPU setup: the artists on my shelf only exist because the studio's weights are mine.

The recipe itself is short. Generate about sixty candidates from a canonical description, click through them in a little keyboard-driven picker (space to keep, arrows to move – the agents built it, I judged), keep the twenty or so where the character is recognizably the same person, caption each with a made-up trigger word, and train overnight on the same GPU. About four hours per character.
